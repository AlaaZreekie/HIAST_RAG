...
from src.rag_chain import get_conversation_aware_response, invoke_llm_with_context
...
class DefaultRAGPipeline(BaseRAGPipeline):
    async def answer_question(self, question: str, conversation_history: Optional[Dict] = None) -> str:
        # For compatibility, run in thread pool
...
class QueryTransformRAGPipeline(BaseRAGPipeline):
    ...
    async def answer_question(self, question: str, conversation_history: Optional[Dict] = None) -> str:
        ...
        # Run all sub-question answers concurrently
        loop = asyncio.get_event_loop()
...
class RAGFusionPipeline(BaseRAGPipeline):
        ...
        async def answer_question(self, question: str, conversation_history: Optional[Dict] = None) -> str:
            ...
            embedder = Embedder()
            loop = asyncio.get_event_loop()
            # Async document retrieval for each sub-question
            ...
            # Sort docs by frequency and take the top 15
