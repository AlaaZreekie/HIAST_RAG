...
import { getCurriculumById } from "@/lib/publicCurriculumsApi";

export const CourseClient = ({ lang, id }) => {
  const [courses, setCourses] = useState([]);

  useEffect(() => {
    async function fetchData() {
      const s = await getCurriculumById(id);
              ...
              >
                ...
                <div className="p-8">
                  <h3 className="text-2xl font-bold mb-4 text-gray-900">
                    {course["Course"]["Translations"][0]["Name"]}
                  </h3>

                  <p className="text-gray-600 mb-6 leading-relaxed">
                    {course["Course"]["Translations"][0]["Description"]}
                  </p>
                  ...
                  <div className="flex-col flex justify-between mb-6">
                    ...
                    <div className="flex items-center gap-2 mb-2 text-blue-600">
                      ...
                      <span className="font-semibold">
                        {"Theoretical Hours: " +
                          course.Course.TheoreticalHours +
                          "H"}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 mb-2 text-red-600">
                      ...
                      <span className="font-semibold">
                        {"Practical Hours: " +
                          course.Course.PracticalHours +
                          "H"}
                      </span>
                    </div>
                    <h3>
                      Description:{" "}
                      {course.Course.Translations.Description || "N/A"}
                    </h3>
                  </div>
                </div>
              </div>
