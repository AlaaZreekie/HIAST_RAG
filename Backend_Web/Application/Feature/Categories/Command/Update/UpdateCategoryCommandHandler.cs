...
{
    public class UpdateCategoryCommandHandler(IAppUnitOfWork unitOfWork) : IRequestHandler<UpdateCategoryCommand>
    {
                ...
                foreach (var transUpdate in request.Translations)
                {
                    ...
                    if (existingTranslation != null)
                    {
                        if (!string.IsNullOrWhiteSpace(transUpdate.Name) && existingTranslation.Name != transUpdate.Name)
                        {
                            ...
                            existingTranslation.Slug = SlugGenerator.Slugify(transUpdate.Name);
                        }
                    }
                }
            }
